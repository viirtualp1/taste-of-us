// PDF: temporarily disabled (Chromium not available on serverless)
...
function computeCookAndResponsible(
  ...
  for (let i = 0; i < menu.length; i++) {
    ...
    effectiveMenu.push({
      ...day,
      _cook: Object.keys(cook).length ? cook : undefined,
    })
  }
...
/* generateMenuPDF HTML template kept for reference — restore when re-enabling PDF
    const html = `
      <!DOCTYPE html>
      <html>
        <head>
